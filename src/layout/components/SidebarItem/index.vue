<template>
    <div v-if="item.meta.sidebar !== false" class="sidebar-item">
        ...
        <el-submenu v-else :title="generateI18nTitle(item.meta.i18n, item.meta.title)" :index="resolvePath(item.path)">
            <template slot="title">
                <template v-if="item.meta.icon">
                    <i v-if="item.meta.icon.indexOf('el-icon-') === 0 || item.meta.icon.indexOf('ri-') === 0" :class="`${item.meta.icon} icon`" />
                    <svg-icon v-else :name="item.meta.icon" class="icon" />
...
.el-menu--inline {
    @include themeify {
        background-color: darken(themed('g-sub-sidebar-bg'), 7) !important;
    }
    .el-menu-item,

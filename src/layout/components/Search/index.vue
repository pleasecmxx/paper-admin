    ...
    <div id="search" :class="{'searching': isShow}" @click="$eventBus.$emit('global-search-toggle')">
        <div class="container">
            <div class="search-box" @click.stop>
                <el-input ref="input" v-model="search" size="default" prefix-icon="el-icon-search" placeholder="搜索页面" clearable @keydown.esc.native="$eventBus.$emit('global-search-toggle')" />
                    ...
                    <a :href="isExternal(item.path) ? item.path : href" class="item" :target="isExternal(item.path) ? '_blank' : '_self'" @click="navigate">
                        <div class="icon">
                            <template v-if="item.icon">
                                <i v-if="item.icon.indexOf('el-icon-') === 0 || item.icon.indexOf('ri-') === 0" :class="`${item.icon}`" />
            ...
            arr.map(item => {
                if (item.meta.sidebar !== false) {
                    if (this.hasChildren(item)) {
                        ...
                        breadcrumb.push(this.generateI18nTitle(item.meta.i18n, item.meta.title))
                        let path = ''

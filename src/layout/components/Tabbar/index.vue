    ...
    >
        <div ref="tabs" class="tabs" @mousewheel.prevent>
            <transition-group name="tabbar" tag="div">
                <div v-for="item in $store.state.tabbar.list" :key="item.path" :class="{
                    'tab': true,
                    'tab-animate': true,
...
export default {
    ...
    watch: {
        ...
        $route: {
            handler(val) {
                this.$store.state.settings.enableTabbar && this.$store.dispatch('tabbar/add', val)
            },
            immediate: true
        }
    },
    ...
    methods: {
        ...
        handlerMouserScroll(event) {
            ...
            this.$refs['tabs'].scrollLeft = this.$refs['tabs'].scrollLeft + step
        },
        // 校验指定标签两侧是否有可关闭的标签

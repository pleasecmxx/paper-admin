...
const mutations = {
    // 根据localstorage数据复原当前帐号的固定标签页
    recoveryStorage(state, account) {
        if (localStorage.getItem('tabbarPinData') != null) {
            state.list = JSON.parse(localStorage.getItem('tabbarPinData'))[account] || []
        }
    },
    // 更新localstorage数据
    updateStorage(state, account) {
        let data = JSON.parse(localStorage.getItem('tabbarPinData')) || {}
        data[account] = state.list.filter(item => {
            return item.isPin
        })
        localStorage.setItem('tabbarPinData', JSON.stringify(data))
    },
    // 清空所有标签页，登出的时候需要清空
    clean(state) {
        state.list = []
